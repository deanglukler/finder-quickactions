...
action_name="_VIDEO_Convert To MP4"
converted_count=0
failed_count=0
...
unique_output_path() {
  ...
  while [[ -e "$candidate" || -L "$candidate" ]]; do
    candidate="$directory/$stem $number.mp4"
    (( number++ ))
  done
...
notify "Converting $# video file(s) to MP4."
...
if (( converted_count > 0 )); then
  message="Converted $converted_count video file(s) to MP4."
